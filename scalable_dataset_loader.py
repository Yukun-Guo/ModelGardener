...
class ScalableDatasetLoader:
    ...
    def _convert_labels_to_categorical(self, images, labels):
        ...
        labels_categorical = tf.one_hot(tf.cast(labels, tf.int32), depth=tf.cast(num_classes, tf.int32))
    ...
    def infer_data_specs(self) -> Tuple[Tuple[int, ...], int]:
        """
        Infer input shape and number of classes from the dataset.
        
        Returns:
            Tuple[Tuple[int, ...], int]: (input_shape, num_classes)
        """
        try:
            ...
            for batch in sample_dataset:
                if isinstance(batch, tuple) and len(batch) == 2:
                    ...
                    input_shape = tuple(images.shape[1:])  # Remove batch dimension
                    ...
                    BRIDGE.log(f"Inferred input shape: {input_shape}")
                    BRIDGE.log(f"Inferred number of classes: {num_classes}")
                    
                    return input_shape, int(num_classes)
